...
def _validate(data: bytes) -> bytes:
    ...
    try:
        ...
        raise ImageGenerationError("서버 응답이 읽을 수 있는 이미지가 아닙니다.") from error
    return data
...
def _decode(encoded: str) -> bytes:
    try:
        ...
        raise ImageGenerationError("이미지의 base64 응답이 올바르지 않습니다.") from error
        ...
        with httpx.stream("GET", url, timeout=30, follow_redirects=True) as response:
            ...
            for chunk in response.iter_bytes():
                ...
                if total > MAX_IMAGE_BYTES:
                    raise ImageGenerationError("이미지가 다운로드 크기 제한을 초과했습니다.")
                chunks.append(chunk)
        return _validate(b"".join(chunks))
    except httpx.HTTPError as error:
        raise ImageGenerationError("생성된 이미지를 다운로드하지 못했습니다.") from error
...
def extract_image(result, download: Callable | None = None) -> bytes:
    ...
    for block in payload.get("content", []):
        ...
        if block.get("type") == "resource":
            ...
            if resource.get("mimeType", "").startswith("image/") and resource.get("blob"):
    ...
    raise ImageGenerationError("MCP 응답에서 이미지 데이터나 다운로드 URL을 찾지 못했습니다.")
...
async def _call_tool(prompt: str):
    ...
    async with (
        ...
        ClientSession(*streams, read_timeout_seconds=timedelta(seconds=timeout)) as session,
    ):
