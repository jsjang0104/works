"""
원본 Whisper-medium으로 음성 인식 평가 (data human 전용)
...
출력:   asr/eval/medium_result_human.json
        asr/eval/medium_eval_human.png
...
DA score vs WER(%) 박스플롯은 IQR 기준 극단치(outlier)를 제외하고 그립니다.

사용법:
    python medium_evaluate.py
"""
...
RESULT_PATH = os.path.join(BASE_DIR, "medium_result_human.json")
PLOT_PATH   = os.path.join(BASE_DIR, "medium_eval_human.png")
...
def drop_outliers(values):
    """1.5*IQR 범위를 벗어나는 극단치를 제외한 값 목록을 반환"""
    if len(values) < 4:
        return values
...
def shared_ylim(key, vals):
    """baseline/final/small/medium 결과를 함께 읽어 그림들이 같은 y축 범위를 갖도록 (lo, hi)를 계산 (0을 기준점으로 포함)"""
    all_vals = list(vals) + [0]
    for prefix in ("baseline", "final", "small", "medium"):
...
def run_human_eval():
    ...
    if n_plots == 2:
        axes = list(axes)
    fig.suptitle("Whisper-medium (original) · Human Speech Evaluation", fontsize=13)
    ...
    if len(all_scores) >= 3:
        ...
        ax.set_title(f"Score vs WER  (ρ={r_wer:.3f}, outlier 제외)")
        ax.set_xticks([1, 2, 3]); ax.grid(alpha=0.3)
