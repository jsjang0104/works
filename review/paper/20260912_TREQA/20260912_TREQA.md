...
## 생각한 것
- 여기서는 sentence level을 document level (문학)로 확장하였음. 
- 우리는 sentence level -> document level도 아닌 document level의 안/밖에 있는 담화 맥락 (화용론)을 접목할 수 있을 것.
- QAG (question-answering-generation)에서 English만 다룸 -> 차별화 포인트 보임
- 기억에 남는 절 (2.2 QA 중): *QG is more open-ended and requires generating diverse and meaningful questions, QA is more constrained task that involves extracting relevant information from the text. Hence, while larger, more capable models may be necessary for high-quality QG, smaller, more efficient models could be sufficient for QA, (중략)* 
    - 뭔가 논리가 그럴싸해서 우리 연구에 적용해볼 수 있을듯. 예를 들어 벤치마크를 synthetic 하는 모델을 더 좋은 걸 쓰고, 그걸 검증하는 모델을 더 별로인 걸 쓰기 -> 벤치마크 효과 극대화..?